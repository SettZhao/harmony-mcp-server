...
- native侧代码入口文件保持napi_init.cpp不变

## 2. API 查询规则

- 查询鸿蒙 API **必须** 使用 MCP 工具，禁止依赖过期的静态文档：
  - `deveco-mcp/harmonyos_knowledge_search` — 按关键词查询鸿蒙云端知识库（已实时更新至 API 22）
- API 查询调用示例：
  ```
...
- 构建工具：`deveco-mcp/build_project`（优先）或 `hvigorw`（终端备用）
- SDK 类型：`HarmonyOS`（`build-profile.json5` 中 `runtimeOS: "HarmonyOS"`）
- 构建顺序：`assembleHar` → `assembleHap` → 安装到设备 → 运行测试
- **每步必须出现 `BUILD SUCCESSFUL` 才能进入下一步**
- 失败时：分析日志 → 修复代码 → 重试当前步骤，不得跳过

## 3.1 构建执行强制原则（行为偏差红线）

> 以下行为是**严重行为偏差**，一旦发生视为任务失败，必须立即纠正：

| ❌ 禁止行为 | ✅ 正确行为 |
|-----------|-----------|
| 创建 `build-instructions.md`、`移植总结.md` 等文档，然后宣告构建阶段完成 | 调用 `deveco-mcp/build_project` 或在 `terminal` 中运行 `hvigorw` 命令，等待真实输出 |
| 以"代码看起来正确"为由跳过编译步骤 | 必须运行编译命令，以输出 `BUILD SUCCESSFUL` 为准 |
| 在未看到 `BUILD SUCCESSFUL` 的情况下进入下一步 | 每步等待输出，失败则原地修复重试 |
| 将"编写构建说明"当作"构建验证"的交付物 | 构建验证的唯一交付物是 `deveco-mcp/build_project` 或终端的成功输出日志 |

**根本原则**：`deveco-mcp/build_project` 工具或 `terminal` 工具是必须调用的，不是可选的。任何以文档生成代替命令执行的行为都是错误的。
...
```powershell
Get-Command hvigorw   # 构建工具（备用）
Get-Command hdc       # 设备连接工具
hdc list targets      # 验证设备已连接
```

优先使用 `deveco-mcp` MCP 工具代替终端命令：

| 任务 | 优先工具 | 备用方式 |
|------|---------|--------|
| 查询鸿蒙 API | `deveco-mcp/harmonyos_knowledge_search` | — |
| ETS 语法检查 | `deveco-mcp/check_ets_files` | — |
| 项目构建 | `deveco-mcp/build_project` | `hvigorw` 终端命令 |
| 启动应用 / 安装到设备 | `deveco-mcp/start_app` | `hdc install` + `hdc shell aa start` |
| 获取页面 UI 树 | `deveco-mcp/get_uidump` | — |
| UI 自动化测试 | `deveco-mcp/execute_uitest` | — |

---

## 8. ArkTS 与 TypeScript 的差异规则（迁移必读）
...
## 迁移策略规则
- 优先使用Arkts接口实现
- 若为性能敏感模块且Arkts接口无法满足需求，才考虑使用Native模块

## 9. deveco-mcp 工具速查

| 工具 | 用途 | 关键参数 |
|------|------|--------|
| `deveco-mcp/harmonyos_knowledge_search` | 查询鸿蒙云端知识库（API 22 实时） | `keywords: string[]` |
| `deveco-mcp/check_ets_files` | 对 ETS 文件进行 ArkTS 静态语法检查 | `files: string[]`（绝对路径列表） |
| `deveco-mcp/build_project` | 编译构建项目，导出 HAR/HAP/APP | `buildTarget: 'hap'\|'app'`, `buildMode: 'debug'\|'release'` |
| `deveco-mcp/start_app` | 构建并推包到模拟器/真机运行 | `hvd`（设备名），`module`，`product` |
| `deveco-mcp/get_uidump` | 获取 App 当前页面的 UI 树 | `mode: 'simple'\|'full'`, `outputDirectory` |
| `deveco-mcp/execute_uitest` | 在 App 页面执行 click/inputText/screenshot 等 | `actionType`, `x`, `y`, `text` 等 |
| `deveco-mcp/create_harmony_project` | 创建鸿蒙模板工程 | `projectPath`, `appName`, `apiLevel` |